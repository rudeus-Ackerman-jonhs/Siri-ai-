...
module.exports = {
  ...
  langs: {
    vi: {
      ...
      reply: "🌿 Réponse de %1 🌿\n┍──━──━──🌿\n | %2\n┕──━──━──🌿\nRéponds à ce message pour répondre à %1 🤲",
    ...
    en: {
      ...
      reply: "🌿 Reply from %1 🌿\n┍──━──━──🌿\n | %2\n┕──━──━──🌿\nReply this message to continue send message to %1 🤲",
  ...
  onStart: async function ({ args, message, event, usersData, threadsData, api, commandName, getLang }) {
    ...
    const formMessage = {
      ...
      attachment: await getStreamsFromAttachment([...event.attachments, ...(event.messageReply?.attachments || [])].filter(item => mediaTypes.includes(item.type)))
    };

    const successIDs = [];
    const failedIDs = [];
    const adminNames = await Promise.all(config.adminBot.map(async item => ({ id: item, name: await usersData.getName(item) })));
    ...
    if (successIDs.length > 0)
      msg2 += getLang("success", successIDs.length, adminNames.filter(item => successIDs.includes(item.id)).map(item => ` <@${item.id}> (${item.name})`).join("\n"));
    if (failedIDs.length > 0) {
      msg2 += getLang("failed", failedIDs.length, failedIDs.map(item => ` <@${item.adminID}> (${adminNames.find(i => i.id === item.adminID)?.name || item.adminID})`).join("\n"));
      log.err("CALL ADMIN", failedIDs);
    }
    return message.reply({ body: msg2, mentions: adminNames.map(item => ({ id: item.id, tag: item.name })) });
  },
